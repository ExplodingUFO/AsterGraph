...
public sealed class GraphEditorSessionTests
{
    ...
    public void IGraphEditorCommands_DefinesHostFacingEditAndViewportActions()
    {
        ...
        AssertMethod(commandsType, nameof(IGraphEditorCommands.TryDeleteSelectedConnections));
        Assert.Equal(typeof(bool), commandsType.GetMethod(nameof(IGraphEditorCommands.TryDeleteSelectedConnections), Type.EmptyTypes)!.ReturnType);
        AssertMethod(commandsType, nameof(IGraphEditorCommands.TrySliceConnections), typeof(GraphPoint), typeof(GraphPoint));
        Assert.Equal(typeof(bool), commandsType.GetMethod(nameof(IGraphEditorCommands.TrySliceConnections), [typeof(GraphPoint), typeof(GraphPoint)])!.ReturnType);
        ...
        AssertMethod(commandsType, nameof(IGraphEditorCommands.TrySetConnectionLabel), typeof(string), typeof(string), typeof(bool));
        ...
        AssertMethod(commandsType, nameof(IGraphEditorCommands.FitSelectionToViewport), typeof(bool));
        AssertMethod(commandsType, nameof(IGraphEditorCommands.FocusSelection), typeof(bool));
    ...
    public void IGraphEditorQueries_DefinesHostFacingSnapshotAndDiscoveryReads()
    {
        ...
        Assert.NotNull(typeof(GraphEditorSelectionSnapshot).GetProperty(nameof(GraphEditorSelectionSnapshot.SelectedConnectionIds)));
        Assert.NotNull(typeof(GraphEditorSelectionSnapshot).GetProperty(nameof(GraphEditorSelectionSnapshot.PrimarySelectedConnectionId)));
        Assert.NotNull(typeof(GraphEditorSelectionChangedEventArgs).GetProperty(nameof(GraphEditorSelectionChangedEventArgs.SelectedConnectionIds)));
        Assert.NotNull(typeof(GraphEditorSelectionChangedEventArgs).GetProperty(nameof(GraphEditorSelectionChangedEventArgs.PrimarySelectedConnectionId)));

        AssertMethod(queriesType, nameof(IGraphEditorQueries.GetSelectedNodeConnectionIds));
        Assert.Equal(
    ...
    public void RuntimeSession_FeatureDescriptors_ExposeExplicitDiscoveryWithoutFacadeInspection()
    {
        ...
        Assert.False(descriptors["capability.layout.align"].IsAvailable);
        Assert.False(descriptors["capability.layout.distribute"].IsAvailable);
    ...
    [Fact]
    public void RuntimeSession_SelectionContextMenuDescriptors_ExposeGroupingAndLayoutActions()
    {
        ...
        var snapGrid = Assert.Single(selectionMenu, item => item.Id == "selection-snap-grid");
        ...
        Assert.True(snapGrid.IsEnabled);
        Assert.Equal("layout.snap-selection", snapGrid.Command!.CommandId);
        Assert.Contains(snapGrid.Command.Arguments, argument => argument.Name == "gridSize" && argument.Value == "20");
        ...
        Assert.Contains(selectionTools, tool => tool.Id == "selection-align-left" && tool.Invocation.CommandId == "layout.align-left" && tool.CanExecute);
        Assert.Contains(selectionTools, tool => tool.Id == "selection-distribute-horizontal" && tool.Invocation.CommandId == "layout.distribute-horizontal" && !tool.CanExecute);
        var snapTool = Assert.Single(selectionTools, tool => tool.Id == "selection-snap-grid");
        Assert.Equal("layout.snap-selection", snapTool.Invocation.CommandId);
        Assert.Contains(snapTool.Invocation.Arguments, argument => argument.Name == "gridSize" && argument.Value == "20");
    }
        ...
        Assert.True(session.Commands.TryExecuteCommand(
            new GraphEditorCommandInvocationSnapshot(
                ...
                [
                    ...
                    new GraphEditorCommandArgumentSnapshot("label", "Preview Flow"),
                    new GraphEditorCommandArgumentSnapshot("updateStatus", "false"),
                ])));

        Assert.Equal(
            "Preview Flow",
            Assert.Single(session.Queries.CreateDocumentSnapshot().Connections).Label);
    ...
    public void RuntimeSession_StockContextMenuDescriptorSignatures_RemainStableAcrossTargets()
    {
        ...
        var expected = """
            canvas:canvas-add-node~Add Node~add~True~-~False~-~[add-category-Tests~Tests~-~True~-~False~-~[add-node-tests-session-stock-menu-signatures~Session Node~node~True~-~False~nodes.add(definitionId=tests.session.stock-menu-signatures,worldX=160,worldY=90)~[-]]]||canvas-sep-1~~-~False~-~True~-~[-]||canvas-fit-view~Fit View~fit~True~-~False~viewport.fit()~[-]||canvas-reset-view~Reset View~reset~True~-~False~viewport.reset()~[-]||canvas-snap-all-grid~Snap All Nodes To Grid~snap~True~-~False~layout.snap-all(gridSize=20)~[-]||canvas-sep-2~~-~False~-~True~-~[-]||canvas-save~Save Snapshot~save~True~-~False~workspace.save()~[-]||canvas-load~Load Snapshot~load~False~No saved snapshot yet. Save once to create one.~False~workspace.load()~[-]||canvas-import-fragment~Import Fragment~import~False~-~False~fragments.import()~[-]||canvas-cancel-pending~Cancel Pending Connection~cancel~False~-~False~connections.cancel()~[-]
            selection:selection-delete~Delete 2 Selected Nodes~delete~True~-~False~selection.delete()~[-]||selection-export~Export Fragment~export~True~-~False~fragments.export-selection()~[-]||selection-create-group~Create Group~group-create~True~-~False~groups.create(title=Group)~[-]||selection-wrap-composite~Wrap Selection To Composite~composite-wrap~True~-~False~composites.wrap-selection()~[-]||selection-sep-1~~-~False~-~True~-~[-]||selection-align~Align~align~True~-~False~-~[selection-align-left~Left~align-left~True~-~False~layout.align-left()~[-]>>selection-align-center~Center~align-center~True~-~False~layout.align-center()~[-]>>selection-align-right~Right~align-right~True~-~False~layout.align-right()~[-]>>selection-align-top~Top~align-top~True~-~False~layout.align-top()~[-]>>selection-align-middle~Middle~align-middle~True~-~False~layout.align-middle()~[-]>>selection-align-bottom~Bottom~align-bottom~True~-~False~layout.align-bottom()~[-]]||selection-distribute~Distribute~distribute~True~-~False~-~[selection-distribute-horizontal~Horizontally~distribute-horizontal~False~Select at least three nodes before distributing.~False~layout.distribute-horizontal()~[-]>>selection-distribute-vertical~Vertically~distribute-vertical~False~Select at least three nodes before distributing.~False~layout.distribute-vertical()~[-]]||selection-snap-grid~Snap Selection To Grid~snap~True~-~False~layout.snap-selection(gridSize=20)~[-]
            node:node-inspect~Inspect Source Node~inspect~True~-~False~nodes.inspect(nodeId=tests.session.source-001)~[-]||node-center~Center View Here~center~True~-~False~viewport.center-node(nodeId=tests.session.source-001)~[-]||node-toggle-surface-expansion~Expand Node Card~expand~True~-~False~nodes.surface.expand(nodeId=tests.session.source-001,expansionState=Expanded)~[-]||node-rotate~Rotate 90 Degrees~rotate~True~-~False~nodes.rotate(nodeId=tests.session.source-001,rotationDegrees=90)~[-]||node-sep-1~~-~False~-~True~-~[-]||node-delete~Delete Node~delete~True~-~False~nodes.delete-by-id(nodeId=tests.session.source-001)~[-]||node-duplicate~Duplicate Node~duplicate~True~-~False~nodes.duplicate(nodeId=tests.session.source-001)~[-]||node-disconnect~Disconnect~disconnect~True~-~False~-~[node-disconnect-in~Incoming~disconnect~True~-~False~connections.disconnect-incoming(nodeId=tests.session.source-001)~[-]>>node-disconnect-out~Outgoing~disconnect~True~-~False~connections.disconnect-outgoing(nodeId=tests.session.source-001)~[-]>>node-disconnect-all~All~disconnect~True~-~False~connections.disconnect-all(nodeId=tests.session.source-001)~[-]]||node-create-connection~Create Connection From~connect~True~-~False~-~[node-connect-tests.session.source-001-out~Output~-~True~-~False~-~[compatible-node-tests.session.target-001~Target Node~-~True~-~False~-~[compatible-port-tests.session.target-001-in~Input~connect~True~-~False~connections.connect(sourceNodeId=tests.session.source-001,sourcePortId=out,targetNodeId=tests.session.target-001,targetPortId=in)~[-]]]]
    ...
    public void FactoryEditorSession_And_CreateSession_ExposeEquivalentSharedCanonicalSignatures()
    {
        ...
        Assert.Contains("nodes.inspect", retainedCommandIds);
        Assert.Contains("nodes.inspect", runtimeCommandIds);
        Assert.Contains("nodes.delete-by-id", retainedCommandIds);
    ...
    public void FactoryEditorSession_ExposesRetainedCommandMetadataThroughSharedDescriptorContract()
    {
        ...
        var runtimeDuplicate = runtimeCommands["nodes.duplicate"];
        Assert.Equal("Duplicate Node", runtimeDuplicate.Title);
        Assert.Equal("nodes", runtimeDuplicate.Group);
        Assert.Equal("duplicate", runtimeDuplicate.IconKey);
        Assert.Null(runtimeDuplicate.DefaultShortcut);
        Assert.Equal(GraphEditorCommandSourceKind.Kernel, runtimeDuplicate.Source);
        Assert.True(runtimeDuplicate.CanExecute);
        ...
        Assert.Equal(runtimeCopy.DisabledReason, copy.DisabledReason);
        ...
        var retainedAlign = retainedCommands["layout.align-left"];
        var runtimeAlign = runtimeCommands["layout.align-left"];
        Assert.Equal("Align Left", retainedAlign.Title);
        Assert.Equal("layout", retainedAlign.Group);
        Assert.Equal("align-left", retainedAlign.IconKey);
        Assert.Equal(GraphEditorCommandSourceKind.Kernel, retainedAlign.Source);
        Assert.Equal(GraphEditorCommandSourceKind.Kernel, runtimeAlign.Source);
    }
