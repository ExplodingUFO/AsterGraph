...
public sealed class ReactFlowParityRoadmapDocsTests
{
    ...
    [Fact]
    public void ParityRoadmapDocs_RecordPhase546PostWhiteboardQueueRefreshInBothLocales()
    {
        var englishParity = ReadRepoFile("docs/en/phase-0-reactflow-parity-audit.md");
        var chineseParity = ReadRepoFile("docs/zh-CN/phase-0-reactflow-parity-audit.md");
        ...
        foreach (var contents in new[] { englishParity, chineseParity })
        {
            Assert.Contains("Phase 546", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #215", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-0l9", contents, StringComparison.Ordinal);
            Assert.Contains("post-Phase-545 whiteboard implementation queue refresh", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("WHITEBOARD_POST_GATE_QUEUE_REFRESH", contents, StringComparison.Ordinal);
            Assert.Contains("Phase 547", contents, StringComparison.Ordinal);
            Assert.Contains("whiteboard primitive model skeleton", contents, StringComparison.OrdinalIgnoreCase);
            ...
            Assert.Contains("stacked after PR #214", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no runtime behavior changes", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no public API changes", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no production model/schema changes", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no renderer-layer changes", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no screenshot manifest expansion", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no pointer coordinator edits", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no full React Flow whiteboard parity", contents, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("Phase 546 implements whiteboard primitives", contents, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("whiteboard primitive model is implemented", contents, StringComparison.OrdinalIgnoreCase);
        }

        AssertPostPhase545Queue(ExtractIssueWaveTable(englishParity));
        AssertPostPhase545Queue(ExtractIssueWaveTable(chineseParity));
        Assert.Contains("Phase 546 records the post-Phase-545 whiteboard implementation queue refresh", englishParity, StringComparison.Ordinal);
    ...
    [Fact]
    public void ParityRoadmapDocs_RecordPhase550WhiteboardPrimitiveCookbookScreenshotGateInBothLocales()
    {
        var englishParity = ReadRepoFile("docs/en/phase-0-reactflow-parity-audit.md");
        var chineseParity = ReadRepoFile("docs/zh-CN/phase-0-reactflow-parity-audit.md");
        ...
        foreach (var contents in new[] { englishParity, chineseParity })
        {
            Assert.Contains("Phase 550", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #223", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-wjr", contents, StringComparison.Ordinal);
            Assert.Contains("whiteboard primitive Cookbook screenshot implementation gate", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("WHITEBOARD_PRIMITIVE_COOKBOOK_SCREENSHOT_GATE", contents, StringComparison.Ordinal);
            Assert.Contains("cookbook-whiteboard-primitive-screenshot-proof", contents, StringComparison.Ordinal);
            Assert.Contains("shell-cookbook-whiteboard-primitive-screenshot-proof", contents, StringComparison.Ordinal);
            Assert.Contains("whiteboard primitive visual proof", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("route metadata", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("shell state", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("non-overlap", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no public drawing API", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no pointer coordinator", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no toolbar", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no eraser", contents, StringComparison.OrdinalIgnoreCase);
            ...
            Assert.Contains("no GraphDocument schema change", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no full React Flow whiteboard parity", contents, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("| TBD | TBD | Phase 550: whiteboard primitive Cookbook screenshot implementation gate", contents, StringComparison.Ordinal);
            Assert.DoesNotContain("whiteboard drawing is fully supported", contents, StringComparison.OrdinalIgnoreCase);
        }

        AssertPostPhase545Queue(ExtractIssueWaveTable(englishParity));
        AssertPostPhase545Queue(ExtractIssueWaveTable(chineseParity));
        Assert.Contains("Phase 550 records the whiteboard primitive Cookbook screenshot implementation gate", englishParity, StringComparison.Ordinal);
        Assert.Contains("Phase 550 记录 whiteboard primitive Cookbook screenshot implementation gate", chineseParity, StringComparison.Ordinal);
    }
    ...
    public void ParityRoadmapDocs_RecordPhase552WhiteboardDrawingToolActivationContractInBothLocales()
    {
        ...
        {
            Assert.Contains("Phase 552", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #226", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-718", contents, StringComparison.Ordinal);
            Assert.Contains("whiteboard drawing tool public activation contract", contents, StringComparison.OrdinalIgnoreCase);
            ...
            Assert.Contains("GraphEditorCommandSourceKind.Host", contents, StringComparison.Ordinal);
            Assert.Contains("no runtime command id", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no pointer gesture capture", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no runtime drawing creation", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no toolbar UI implementation", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no eraser behavior", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no persistence/schema changes", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no renderer rewrite", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no screenshot manifest expansion", contents, StringComparison.OrdinalIgnoreCase);
        ...
        AssertPostPhase551Queue(ExtractIssueWaveTable(englishParity));
        AssertPostPhase551Queue(ExtractIssueWaveTable(chineseParity));
        Assert.Contains("Phase 552 records the whiteboard drawing tool public activation contract", englishParity, StringComparison.Ordinal);
        Assert.Contains("Phase 552 记录 whiteboard drawing tool public activation contract", chineseParity, StringComparison.Ordinal);
    }
    ...
    public void ParityRoadmapDocs_RecordPhase557WhiteboardAnnotationStoreContractGateInBothLocales()
    {
        ...
        foreach (var contents in new[] { englishParity, chineseParity })
        {
            Assert.Contains("Phase 557", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #237", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-zfe", contents, StringComparison.Ordinal);
            Assert.Contains("whiteboard annotation store contract gate", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("WHITEBOARD_ANNOTATION_STORE_CONTRACT_GATE", contents, StringComparison.Ordinal);
            Assert.Contains("separate annotation-store contract", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("store ownership", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("store lifetime", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("annotation identity", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("primitive reference", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("style/geometry serialization", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("edit lifecycle", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("migration metadata", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("read/write API boundary", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("GraphDocumentCompatibility", contents, StringComparison.Ordinal);
            Assert.Contains("WorkspacePersistence", contents, StringComparison.Ordinal);
            Assert.Contains("ClipboardFragment", contents, StringComparison.Ordinal);
            Assert.Contains("SceneExportArtifact", contents, StringComparison.Ordinal);
        ...
        {
            Assert.Contains("| #237 | `avalonia-node-map-zfe` | Phase 557: whiteboard annotation store contract gate", table, StringComparison.Ordinal);
            Assert.Contains("Depends on Phase 556", table, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no production annotation store", table, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("| TBD | TBD | Phase 557", table, StringComparison.Ordinal);
        }

        Assert.Contains("Phase 557 records the whiteboard annotation store contract gate", englishParity, StringComparison.Ordinal);
        Assert.Contains("Phase 557 记录 whiteboard annotation store contract gate", chineseParity, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void ParityRoadmapDocs_RecordPhase560InMemoryAnnotationStoreAdapterProofInBothLocales()
    {
        var englishParity = ReadRepoFile("docs/en/phase-0-reactflow-parity-audit.md");
        var chineseParity = ReadRepoFile("docs/zh-CN/phase-0-reactflow-parity-audit.md");
        ...
        foreach (var contents in new[] { englishParity, chineseParity })
        {
            Assert.Contains("Phase 560", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #242", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-nia", contents, StringComparison.Ordinal);
            Assert.Contains("IN_MEMORY_WHITEBOARD_ANNOTATION_STORE_ADAPTER_PROOF", contents, StringComparison.Ordinal);
            Assert.Contains("InMemoryGraphWhiteboardAnnotationStoreBoundary", contents, StringComparison.Ordinal);
            Assert.Contains("session-scoped", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("snapshot create/read/update/delete", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no file/database I/O", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no GraphDocument schema change", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no schema version bump", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no workspace persistence behavior change", contents, StringComparison.OrdinalIgnoreCase);
        ...
        foreach (var table in new[] { ExtractIssueWaveTable(englishParity), ExtractIssueWaveTable(chineseParity) })
        {
            Assert.Contains("| #242 | `avalonia-node-map-nia` | Phase 560: in-memory whiteboard annotation store adapter proof", table, StringComparison.Ordinal);
            Assert.Contains("Depends on Phase 559", table, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("in-memory adapter proof only", table, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("| TBD | TBD | Phase 560", table, StringComparison.Ordinal);
            Assert.DoesNotContain("annotation persistence is implemented", table, StringComparison.OrdinalIgnoreCase);
        }

        Assert.Contains("Phase 560 records the in-memory whiteboard annotation store adapter proof", englishParity, StringComparison.Ordinal);
        Assert.Contains("Phase 560 记录 in-memory whiteboard annotation store adapter proof", chineseParity, StringComparison.Ordinal);
    }
    ...
    public void ParityRoadmapDocs_RecordPhase562ClipboardExportSerializationBoundaryInBothLocales()
    {
        ...
        foreach (var contents in new[] { englishParity, chineseParity })
        {
            Assert.Contains("Phase 562", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #244", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-0gi", contents, StringComparison.Ordinal);
            Assert.Contains("WHITEBOARD_ANNOTATION_CLIPBOARD_EXPORT_SERIALIZATION_BOUNDARY", contents, StringComparison.Ordinal);
            Assert.Contains("annotation identity/reference", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("primitive payload eligibility", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("clipboard fragments", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("scene export artifacts", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("workspace sidecar persistence", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("compatibility and migration proof required before serialization behavior changes", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no production serialization implementation", contents, StringComparison.OrdinalIgnoreCase);
            ...
            Assert.Contains("no saved whiteboard primitive state", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("no full React Flow whiteboard parity", contents, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("whiteboard annotations are serialized to clipboard", contents, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("scene export now includes whiteboard annotations", contents, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("clipboard/export now writes annotations", contents, StringComparison.OrdinalIgnoreCase);
        }

        foreach (var table in new[] { ExtractIssueWaveTable(englishParity), ExtractIssueWaveTable(chineseParity) })
        {
            Assert.Contains("| #244 | `avalonia-node-map-0gi` | Phase 562: whiteboard annotation clipboard/export serialization boundary", table, StringComparison.Ordinal);
            Assert.Contains("Depends on Phase 559", table, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("boundary gate only", table, StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain("| TBD | TBD | Phase 562", table, StringComparison.Ordinal);
            Assert.DoesNotContain("annotation serialization is implemented", table, StringComparison.OrdinalIgnoreCase);
        }

        Assert.Contains("Phase 562 records the whiteboard annotation clipboard/export serialization boundary", englishParity, StringComparison.Ordinal);
        Assert.Contains("Phase 562 记录 whiteboard annotation clipboard/export serialization boundary", chineseParity, StringComparison.Ordinal);
    }
    ...
    private static void AssertPostPhase545Queue(string table)
    {
        ...
        Assert.Contains("| #223 | `avalonia-node-map-wjr` | Phase 550: whiteboard primitive Cookbook screenshot implementation gate", table, StringComparison.Ordinal);
        ...
        Assert.Contains("whiteboard primitive visual proof route", table, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("Stacked after PR #222", table, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("do not merge before Phase 549", table, StringComparison.OrdinalIgnoreCase);
    ...
    private static void AssertWhiteboardLassoEraserFeasibilityAudit(string table)
    {
        ...
        Assert.Contains("NodeCanvas.WhiteboardDrawingMode", table, StringComparison.Ordinal);
        Assert.Contains("NodeCanvasWhiteboardDrawingMode.Rectangle", table, StringComparison.Ordinal);
        Assert.Contains("NodeCanvasWhiteboardDrawingMode.Freehand", table, StringComparison.Ordinal);
