...
public sealed class AccessibilityManualValidationDocsTests
{
    ...
    [Fact]
    public void AccessibilityManualValidationDocs_RecordPhase517PlatformEquivalentLiveAtEvidence()
    {
        var englishParity = ReadRepoFile("docs/en/phase-0-reactflow-parity-audit.md");
        var chineseParity = ReadRepoFile("docs/zh-CN/phase-0-reactflow-parity-audit.md");
        var englishRecipe = ReadRepoFile("docs/en/hosted-accessibility-recipe.md");
        ...
        foreach (var contents in new[] { englishParity, chineseParity })
        {
            Assert.Contains("Phase 517", contents, StringComparison.Ordinal);
            Assert.Contains("GitHub #156", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-1pd", contents, StringComparison.Ordinal);
            Assert.Contains("ACCESSIBILITY_LIVE_AT_UIA_EVIDENCE", contents, StringComparison.Ordinal);
            Assert.Contains("Windows UI Automation", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("platform-equivalent", contents, StringComparison.OrdinalIgnoreCase);
        ...
        foreach (var contents in new[] { englishRecipe, chineseRecipe })
        {
            ...
            Assert.Contains("avalonia-node-map-1pd", contents, StringComparison.Ordinal);
            Assert.Contains("Windows UI Automation", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("validation-prevent-cycle", contents, StringComparison.Ordinal);
            Assert.Contains("GraphEditorView", contents, StringComparison.Ordinal);
            Assert.Contains("NodeCanvas", contents, StringComparison.Ordinal);
            Assert.Contains("projected host command buttons", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("PART_CommandPaletteSearchBox", contents, StringComparison.Ordinal);
            Assert.Contains("PART_ParameterSearchBox", contents, StringComparison.Ordinal);
            Assert.Contains("validation/export/status", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("not exposed", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("live screen-reader speech output was not observed", contents, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("GitHub #158", contents, StringComparison.Ordinal);
            Assert.Contains("avalonia-node-map-g0u", contents, StringComparison.Ordinal);
            Assert.Contains("no live-region/runtime behavior change", contents, StringComparison.OrdinalIgnoreCase);
