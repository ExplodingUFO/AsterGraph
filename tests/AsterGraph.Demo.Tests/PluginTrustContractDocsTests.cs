...
public sealed class PluginTrustContractDocsTests
{
    ...
    public void PluginTrustContractDocs_DefendManifestTrustPolicyAndImplicitAllowContract()
    {
        ...
        Assert.Contains("## Trust Review Short Path", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("AsterGraph.PluginTool validate", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("PluginTool validation is evidence for host policy.", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("not a marketplace approval, a sandbox decision, or an automatic load authorization", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("## Host Policy Examples", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("Allow all local dev", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("Allow by hash", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("Allow by manifest or publisher", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("Block unknown source", pluginTrustContractsEn, StringComparison.Ordinal);
        Assert.Contains("Enterprise fixed plugin directory", pluginTrustContractsEn, StringComparison.Ordinal);
    ...
    public void PluginTrustContractDocs_DefendHostIntegrationPluginRecipeAndConsumerSampleLinks()
    {
        ...
        Assert.Contains("## Trust Review Short Path", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("AsterGraph.PluginTool validate", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("PluginTool validation 是给 host policy 使用的证据", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("不是 marketplace approval、sandbox decision，也不是自动加载授权", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("## Host Policy Examples", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("Allow all local dev", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("Allow by hash", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("Allow by manifest or publisher", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("Block unknown source", pluginTrustContractsZh, StringComparison.Ordinal);
        Assert.Contains("Enterprise fixed plugin directory", pluginTrustContractsZh, StringComparison.Ordinal);
