...
public sealed class GraphSceneSvgExportService : IGraphSceneSvgExportService
{
    ...
    public string Export(GraphEditorSceneSnapshot scene, string? path = null)
    {
        ...
        File.WriteAllText(resolvedPath, BuildSvg(scene), Encoding.UTF8);
        return resolvedPath;
    }

    private static string BuildSvg(GraphEditorSceneSnapshot scene)
    {
        ...
        builder.AppendLine("""  <rect width="100%" height="100%" fill="#09141C" />""");
