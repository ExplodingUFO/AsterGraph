...
| Marquee/box selection and multi-select | Present | `GetSelectionRectangleSnapshot`, overlay coordinator marquee selection, `SelectAll`, `SelectNone`, and `InvertSelection` have tests and Cookbook routes. This is rectangular marquee selection, not freehand lasso or whiteboard drawing parity. | Keep guarded. |
| Whiteboard / lasso / eraser / rectangle draw | Gap retained / feasibility scoped | React Flow whiteboard docs list Freehand draw, Lasso selection, Eraser, and Rectangle draw as distinct advanced examples. AsterGraph currently has rectangle marquee selection and command projection evidence, but no whiteboard primitives or drawing tools. | Phase 529 records the feasibility boundary without implementing whiteboard behavior. |
...
Phase 529 records the whiteboard/lasso/eraser feasibility audit through GitHub #181 / `avalonia-node-map-jrm`. React Flow documents whiteboard features as a separate advanced track at https://reactflow.dev/learn/advanced-use/whiteboard, including Freehand draw, Lasso selection, Eraser, and Rectangle draw. AsterGraph's current evidence is narrower: rectangular marquee selection and selection command projection are guarded, but this does not become full React Flow whiteboard parity or a commitment to additional runtime/API/UI/renderer behavior in this slice.
...
| Lasso/freehand selection | Gap retained | No source-backed lasso/freehand selection mode, partial/freeform inclusion policy, or lasso screenshot route is documented. | Needs a later behavior/API issue for pointer-mode state machine, freeform geometry capture, hit-testing, and selection semantics. Overall Phase 529 boundary remains not full React Flow whiteboard parity. |
