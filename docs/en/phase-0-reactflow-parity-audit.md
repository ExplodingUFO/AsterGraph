...
| Marquee/box selection and multi-select | Present | `GetSelectionRectangleSnapshot`, `GetSelectionLassoSnapshot`, overlay coordinator marquee selection, internal lasso bridge selection, internal lasso gesture capture, public `NodeCanvasSelectionMode.Lasso`, `SelectAll`, `SelectNone`, and `InvertSelection` have tests and Cookbook routes. Rectangle marquee is UI-backed and remains the default; lasso/freehand has backend/editor query support, an internal Avalonia bridge, internal pointer gesture capture, and a public `NodeCanvas.SelectionMode` activation path. | Keep guarded. |
| Whiteboard / lasso / eraser / rectangle draw | Gap retained / public lasso selection started | React Flow whiteboard docs list Freehand draw, Lasso selection, Eraser, and Rectangle draw as distinct advanced examples. AsterGraph now has rectangle marquee selection, command projection evidence, a guarded backend `GetSelectionLassoSnapshot(...)` query, an internal Avalonia lasso selection bridge, internal lasso gesture capture, and public lasso pointer-mode activation, but no toolbar UX, visual gesture capture, whiteboard primitives, or drawing tools. | Phase 533 exposes `NodeCanvasSelectionMode.Lasso` and routes real pointer gestures through existing lasso selection without implementing full whiteboard UI behavior. |
