...
| Marquee/box selection and multi-select | Present | `GetSelectionRectangleSnapshot`, `GetSelectionLassoSnapshot`, overlay coordinator marquee selection, `SelectAll`, `SelectNone`, and `InvertSelection` have tests and Cookbook routes. Rectangle marquee is UI-backed; lasso/freehand is currently a backend/editor query only. | Keep guarded. |
| Whiteboard / lasso / eraser / rectangle draw | Gap retained / backend lasso query started | React Flow whiteboard docs list Freehand draw, Lasso selection, Eraser, and Rectangle draw as distinct advanced examples. AsterGraph now has rectangle marquee selection, command projection evidence, and a guarded backend `GetSelectionLassoSnapshot(...)` query, but no UI pointer mode, whiteboard primitives, or drawing tools. | Phase 530 starts the lasso query contract without implementing whiteboard UI behavior. |
...
| Lasso/freehand selection | Backend query supported / UI gap retained | `GetSelectionLassoSnapshot`, `GraphEditorSelectionLassoSnapshot`, `Queries_GetSelectionLassoSnapshot_ReturnsCenterContainedNodesAndConnections`, `Queries_GetSelectionLassoSnapshot_UsesNodeCenterInsteadOfBoundsIntersection`, and `Queries_GetSelectionLassoSnapshot_WithOpenOrDegeneratePath_ReturnsDeterministicResults` prove the active-scope backend query. The query treats freeform points as a closed polygon, selects nodes by center-point containment, and selects connections when both endpoint nodes are selected. | No Avalonia pointer-mode state machine, gesture capture, lasso screenshot route, eraser behavior, drawing primitives, persistence, or renderer layer is implemented. Overall Phase 530 boundary remains not full React Flow whiteboard parity. |
