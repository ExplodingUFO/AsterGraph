...
| Marquee/box selection and multi-select | Present | `GetSelectionRectangleSnapshot`, `GetSelectionLassoSnapshot`, overlay coordinator marquee selection, internal lasso bridge selection, `SelectAll`, `SelectNone`, and `InvertSelection` have tests and Cookbook routes. Rectangle marquee is UI-backed; lasso/freehand has backend/editor query support plus an internal Avalonia bridge, but not public pointer-mode UI. | Keep guarded. |
| Whiteboard / lasso / eraser / rectangle draw | Gap retained / internal lasso bridge started | React Flow whiteboard docs list Freehand draw, Lasso selection, Eraser, and Rectangle draw as distinct advanced examples. AsterGraph now has rectangle marquee selection, command projection evidence, a guarded backend `GetSelectionLassoSnapshot(...)` query, and an internal Avalonia lasso selection bridge, but no public UI pointer mode, whiteboard primitives, or drawing tools. | Phase 531 bridges lasso selection finalization without implementing full whiteboard UI behavior. |
