...
| `workbench.command-projection` | Workbench | Beta | shared command descriptors plus hosted action projection | Supported | Partial | `ConsumerSample.Avalonia` | `COMMAND_PROJECTION_UNIFIED_OK:True`, `COMMAND_PALETTE_OK:True`, `TOOLBAR_DESCRIPTOR_OK:True`, `CONTEXT_MENU_DESCRIPTOR_OK:True`, `COMMAND_DISABLED_REASON_OK:True` | command surface refresh budget defended | [Consumer Sample](./consumer-sample.md), [Quick Start](./quick-start.md) |
