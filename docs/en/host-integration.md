...
`DECLARATIVE_HOST_COMPOSITION_GATE`: the selected future candidate is a future composition profile over the existing builder and factory seams. It would package named hosted recipes, default workbench choices, and pass-through policy presets around `AsterGraphHostBuilder.Create(...).BuildAvaloniaView()` while still delegating to `AsterGraphEditorFactory.CreateSession(...)`, `AsterGraphEditorFactory.Create(...)`, and `AsterGraphAvaloniaViewFactory.Create(...)`. That future composition profile is not current public API. It does not widen today's support claims, add a source generator, add a XAML extension, provide React hook parity, or create a <ReactFlow>-equivalent declarative DSL.
...
- scene export: `IGraphSceneSvgExportService` owns non-workspace SVG output built from `IGraphEditorSession.Queries.GetSceneSnapshot()`; raster PNG/JPEG image export can report `GraphEditorSceneImageExportProgressSnapshot` values, honor `GraphEditorSceneImageExportOptions.CancellationToken`, and explicitly scope to the full scene or selected nodes
