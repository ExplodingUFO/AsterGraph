...
| Thin definition builders | Stable canonical | `NodeDefinitionBuilder`, `PortDefinitionBuilder`, `NodeParameterDefinitionBuilder`, `ImplicitConversionRuleBuilder` | Convenience constructors only; each builder terminates in existing DTOs and does not create a second authoring schema or runtime model. |
