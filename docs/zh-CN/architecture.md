...
Public runtime contracts 必须保持无 Avalonia 类型、无 retained view-model 类型。当前 compatibility exception 按精确 symbol 跟踪，而不是按 namespace 放行：`GraphEditorSession(GraphEditorViewModel, ...)`、`GraphEditorViewModel`、`GraphEditorView` 和 retained hosted factory routes。它们只作为迁移桥保留；新宿主应从 `CreateSession(...)`、`IGraphEditorSession` 以及 `GetCompatiblePortTargets(...)` 这类 DTO query 开始。
