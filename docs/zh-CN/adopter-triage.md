...
在把某条反馈当成“应该扩大支持面”的候选之前，先看 [Project Status](./project-status.md) 里的就绪门禁。落在已证明/受边界约束之外的报告，先当 intake evidence，而不是自动扩大范围。若 route 能产出 bundle，就把 `SUPPORT_BUNDLE_PATH:...` 那一行当成附件备注；否则记录 `NO_SUPPORT_BUNDLE:route-cannot-produce-one`。
