...
本仓库采用 canonical-first 模式：稳定语义由 `AsterGraph.Editor` 的 `CreateSession(...)`-based 会话 API 定义。hosted helper 只负责把这条路线组合成 stock UI，retained surface 只用于迁移，已退役的 compatibility-only shim 由 public inventory 的 v1 removal policy 跟踪，internal-only 实现细节不是 public contract。
...
v1 primary surface 中没有。已退役 compatibility-only symbol 及其 replacement 见 [Public API Inventory](./public-api-inventory.md#v1-compatibility-removal-policy)。
...
- v1 primary surface 不允许 compatibility-only API，除非 public inventory 明确列出 symbol、replacement 与 retirement boundary。
- 临时 compatibility-only API 必须带有明确的替代符号路径（replacement path）。
- 不得突发性移除：任何移除都必须先经过明确的弃用周期（包含去向迁移说明与按符号级别的迁移指引）。
- 迁移指引按符号给出旧→新映射：
  - 已退役 compatible-target shim → `AsterGraphEditorFactory.CreateSession(...).GetCompatiblePortTargets(...)`
  - 已退役 MVVM target shape → `AsterGraphEditorFactory.CreateSession(...).GetCompatiblePortTargets(...)` 返回的 `GraphEditorCompatiblePortTargetSnapshot`
