...
- `Performance`：ScaleSmoke budgets、hosted metrics、export budgets 和 graph-size proof summaries。
...
| `workbench.friction-evidence` | Workbench | Beta | v0.64 hosted workflow friction evidence，覆盖 layout resume、node discovery、command feedback 和 support triage | Supported | NotApplicable | `ConsumerSample.Avalonia` | `WORKBENCH_FRICTION_EVIDENCE_OK:True`、`WORKBENCH_FRICTION_PRIORITIZATION_OK:True`、`WORKBENCH_FRICTION_SCOPE_BOUNDARY_OK:True` | 仅本地合成证据；不是外部 adopter reports、telemetry、runtime route、WPF parity 或 GA readiness | [Consumer Sample](./consumer-sample.md)、[Support Bundle](./support-bundle.md) |
| `workbench.affordance-polish` | Workbench | Beta | 从本地 friction evidence 选择的有界 `layout-resume` affordance | Supported | NotApplicable | `ConsumerSample.Avalonia` | `WORKBENCH_AFFORDANCE_POLISH_OK:True`、`WORKBENCH_AFFORDANCE_ROUTE_OK:True`、`WORKBENCH_AFFORDANCE_SCOPE_BOUNDARY_OK:True` | 只走 hosted workbench options route；不是 runtime route、WPF parity、remote sync、macro/query system、execution engine 或 GA claim | [Consumer Sample](./consumer-sample.md)、[Support Bundle](./support-bundle.md) |
| `workbench.evidence-bundle` | Workbench | Beta | 携带 friction rows 和 selected affordance evidence 的本地 support-bundle attachment | Supported | NotApplicable | `ConsumerSample.Avalonia` | `WORKBENCH_FRICTION_SUPPORT_BUNDLE_OK:True`、`WORKBENCH_ADOPTER_EVIDENCE_ATTACHMENT_OK:True`、`WORKBENCH_EVIDENCE_SCOPE_BOUNDARY_OK:True` | 仅本地 evidence attachment；不是 telemetry、remote sync、external validation 或 GA readiness | [Support Bundle](./support-bundle.md) |
| `authoring.repair-help-review` | Authoring | Beta | Problems Panel validation issues 连接到 inspector 同一套 metadata-backed help target，并携带有界 repair evidence | Supported | NotApplicable | `ConsumerSample.Avalonia`、`Editor.Tests` | `GRAPH_ERROR_HELP_TARGET_OK:True`、`GRAPH_PROBLEM_INSPECTOR_HELP_TARGET_OK:True`、`REPAIR_HELP_REVIEW_LOOP_OK:True` | 只覆盖 validation/repair/help review loop；不是自动图修复、执行、telemetry 或 GA support | [Support Bundle](./support-bundle.md)、[Authoring Inspector Recipe](./authoring-inspector-recipe.md) |
| `workbench.adopter-polish-handoff` | Workbench | Beta | v0.64 handoff，汇总 friction evidence、bounded affordance 和本地 support-bundle evidence | Supported | NotApplicable | `ConsumerSample.Avalonia` | `WORKBENCH_ADOPTER_POLISH_HANDOFF_OK:True`、`WORKBENCH_ADOPTER_POLISH_SCOPE_BOUNDARY_OK:True`、`V064_MILESTONE_PROOF_OK:True` | 仅本地 hosted workbench evidence；不是 GA、WPF parity、remote sync、marketplace、sandbox、macro/query system 或 execution engine scope | [Consumer Sample](./consumer-sample.md)、[Project Status](./project-status.md) |
