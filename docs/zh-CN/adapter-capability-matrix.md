...
WPF proof sample evidence 由 `ADAPTER2_WPF_SAMPLE_PROOF_OK:True`、`ADAPTER2_CANONICAL_ROUTE_OK:True` 和 `ADAPTER2_SAMPLE_SCOPE_BOUNDARY_OK:True` 证明；这些 marker 让 adapter-2 release evidence 保持可复现，但不会把它变成第二条 onboarding route。
...
| Hosted full editor shell | `Supported` | `Partial` | `templates/astergraph-avalonia`、`src/AsterGraph.Demo`、`adapter-2 validation evidence`、`adapter-2 validation evidence` | 宿主负责组装 shell；当 stock 级别不足时将 `AsterGraph.Wpf.Controls.GraphEditorView`（来自 `AsterGraphWpfViewFactory`）当普通内容控件使用。 |
| Standalone surfaces | `Supported` | `Partial` | Avalonia 有独立 canvas/inspector/minimap 工厂；WPF 目前仅在 `adapter-2 validation evidence` 与 `adapter-2 validation evidence` 里验证 `GraphEditorView` | 通过 `IGraphEditorSession` 快照在 host 层自建 canvas / inspector / minimap 类表面。 |
...
| WPF proof sample evidence | `Supported` | `Supported` | `ADAPTER2_WPF_SAMPLE_PROOF_OK`、`ADAPTER2_CANONICAL_ROUTE_OK`、`ADAPTER2_SAMPLE_SCOPE_BOUNDARY_OK` 和 `HELLOWORLD_WPF_OK` | 无。这一行让 adapter-2 release evidence 在 canonical route 上保持可复现；它不是第二条 onboarding path（second onboarding path）。 |
