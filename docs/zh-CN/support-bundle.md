...
command palette proof 仍然走 shared command/session route：`COMMAND_PALETTE_GROUPING_OK:True`、`COMMAND_PALETTE_DISABLED_REASON_OK:True` 和 `COMMAND_PALETTE_RECENT_ACTIONS_OK:True` 只证明分组发现、disabled reason 可见性和有界 recent actions，不代表 command macro engine 或 scripting UI。
