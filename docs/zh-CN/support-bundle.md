...
interaction feedback proof 还应该包含 `INTERACTION_FEEDBACK_OK:True`、`CANVAS_FOCUS_RECOVERY_OK:True`、`CONSUMER_GESTURE_PROOF_OK:True` 和 `INTERACTION_SUPPORT_BUNDLE_OK:True`，证明可见 affordance、gesture freshness、command feedback、support-bundle coverage 和 focus recovery 证据仍走现有 canvas、command 和 navigation 路线。
