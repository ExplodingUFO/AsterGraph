...
- 在 release messaging 里重复下一条 `0.xx` alpha/beta 线的 handoff：继续优先做可复制的 host-owned 参数/元数据打磨；只有当 3 到 5 条真实外部反馈在同一个受限风险上聚焦时，才考虑把 defended large-tier performance 或更宽的参数/元数据编辑提上来
