...
| 5 分钟 | 生成 `dotnet new astergraph-avalonia`，运行 starter，再验证 `ConsumerSample.Avalonia -- --proof --support-bundle <support-bundle-path>`。 | 已经拿到 `FIVE_MINUTE_ONBOARDING_OK:True`、`ONBOARDING_CONFIGURATION_OK:True`、`AUTHORING_FLOW_HANDOFF_OK:True` 和 `EXPERIENCE_SCOPE_BOUNDARY_OK:True`。 |
...
Command palette proof 继续走同一条 shared command/session route：期待 `COMMAND_PALETTE_GROUPING_OK:True`、`COMMAND_PALETTE_DISABLED_REASON_OK:True` 和 `COMMAND_PALETTE_RECENT_ACTIONS_OK:True`，不新增 macro 或 scripting。
Navigation proof 继续作为宿主自管层叠在已有 selection、scope 和 viewport command 上：期待 `NAVIGATION_HISTORY_OK:True`、`SCOPE_BREADCRUMB_NAVIGATION_OK:True` 和 `FOCUS_RESTORE_OK:True`，不新增 runtime navigation API。
