...
viewport LOD marker 证明 selected/hovered affordances 仍受 hosted workbench policy 约束：`VIEWPORT_LOD_POLICY_OK:True`、`SELECTED_HOVERED_ADORNER_SCOPE_OK:True`、`LARGE_GRAPH_BALANCED_UX_OK:True` 和 `VIEWPORT_LOD_SCOPE_BOUNDARY_OK:True`。
...
v0.63 workbench discoverability handoff marker 汇总 layout presets、panel state、unified discovery 和 recents/favorites proof，但不创建 runtime route、macro/query system、remote sync、marketplace state、WPF parity 或 GA claim：`WORKBENCH_DISCOVERABILITY_HANDOFF_OK:True`、`WORKBENCH_DISCOVERABILITY_SCOPE_BOUNDARY_OK:True` 和 `V063_MILESTONE_PROOF_OK:True`。
...
- 一组 hosted workbench friction evidence：为 layout resume、node discovery、command feedback 和 support triage 记录带优先级的本地合成行；这些行仍停留在现有 hosted/session route 上，不是外部 adopter report
- 一个 bounded workbench affordance polish 动作：把最高优先级的 `layout-resume` friction row 映射成现有 workbench options 上的 hosted reset-layout action
- 一个本地 support-bundle evidence payload：序列化 workbench friction rows 和选中的 affordance，但不暗示 telemetry、remote sync 或外部 adopter validation
...
- `WORKBENCH_AFFORDANCE_POLISH_OK:True`
- `WORKBENCH_AFFORDANCE_ROUTE_OK:True`
- `WORKBENCH_AFFORDANCE_SCOPE_BOUNDARY_OK:True`
...
- 选中节点参数读写 seam：只通过 `IGraphEditorSession.Queries.GetSelectedNodeParameterSnapshots()` 读取当前选中节点参数，并只通过 `IGraphEditorSession.Commands.TrySetSelectedNodeParameterValue(...)` 写回
- snippet catalog 与插入 seam：把 `consumer.sample.snippet.queue-lane` 和其他 snippet 保持为宿主自管，在宿主里提供 search/preview/favorite/recent，再通过 `StartConnection(...)` 加 `TryCreateConnectedNodeFromPendingConnection(...)` 插入
...
- snippet seam：通过一个小 catalog 暴露宿主自管 snippet，在宿主里提供 search/preview/favorite/recent，用已有 pending-connection command path 插入，并期待 `GRAPH_SNIPPET_CATALOG_OK:True`、`GRAPH_SNIPPET_INSERT_OK:True`、`FRAGMENT_LIBRARY_SEARCH_OK:True`、`FRAGMENT_LIBRARY_PREVIEW_OK:True`、`FRAGMENT_LIBRARY_RECENTS_FAVORITES_OK:True` 和 `FRAGMENT_LIBRARY_SCOPE_BOUNDARY_OK:True`
- default workbench seam：当 stock toolbar、command palette、stencil、inspector、mini-map、fragment、diagnostics 和 status chrome 已经足够时，使用 `AsterGraphHostBuilder.UseDefaultWorkbench()`；期待 `WORKBENCH_DEFAULTS_OK:True`、`WORKBENCH_HOST_BUILDER_HANDOFF_OK:True`、`WORKBENCH_PERFORMANCE_MODE_OK:True`、`BALANCED_MODE_DEFAULT_OK:True`、`WORKBENCH_LOD_POLICY_OK:True`、`PERFORMANCE_MODE_SCOPE_BOUNDARY_OK:True`、`WORKBENCH_LAYOUT_PRESETS_OK:True`、`WORKBENCH_LAYOUT_RESET_OK:True`、`PANEL_STATE_PERSISTENCE_OK:True` 和 `WORKBENCH_SCOPE_BOUNDARY_OK:True`
- discovery seam：让 stencil template、宿主 snippet、graph search result、plugin gallery entry 和 command palette action 都继续通过已有 hosted/session route 发现，并展示 source label；期待 `UNIFIED_DISCOVERY_SURFACE_OK:True`、`DISCOVERY_SOURCE_LABELS_OK:True` 和 `DISCOVERY_COMMAND_ROUTE_OK:True`
- recents/favorites seam：把 node、fragment、command 和 plugin/source 的最近使用与收藏保留在有界 host-owned state，并输出到 support bundle；期待 `WORKBENCH_RECENTS_OK:True`、`WORKBENCH_FAVORITES_OK:True` 和 `RECENTS_FAVORITES_SUPPORT_BUNDLE_OK:True`
...
- edge interaction markers：把 `EDGE_INTERACTION_CACHE_OK:True`、`EDGE_DRAG_ROUTE_SIMPLIFICATION_OK:True`、`SELECTED_EDGE_FEEDBACK_OK:True` 和 `EDGE_RENDERING_SCOPE_BOUNDARY_OK:True` 与现有 edge quick-tool、toolbar 和 geometry 证据放在一起看
- panel projection markers：把 `MINIMAP_LIGHTWEIGHT_PROJECTION_OK:True`、`INSPECTOR_NARROW_PROJECTION_OK:True`、`LARGE_GRAPH_PANEL_SCOPE_OK:True` 和 `PROJECTION_PERFORMANCE_EVIDENCE_OK:True` 与 hosted mini-map policy、选中参数 inspector 证据和现有 projection metrics 放在一起看
- v0.59 handoff markers：把 `LARGE_GRAPH_UX_HANDOFF_OK:True`、`LARGE_GRAPH_UX_SCOPE_BOUNDARY_OK:True` 和 `V059_MILESTONE_PROOF_OK:True` 与 phases 371-374 proof clusters 放在一起看
