...
- 一条宿主自管 navigation proof handoff，包含 `NAVIGATION_HISTORY_OK:True`、`SCOPE_BREADCRUMB_NAVIGATION_OK:True` 和 `FOCUS_RESTORE_OK:True`，不引入 command macro engine、后台图索引、WPF parity 或新的 runtime navigation API
...
- navigation productivity：把 back/forward history、scope breadcrumbs 和 focus restore 保持为宿主自管，并继续复用已有 selection/scope/viewport command，期待 `NAVIGATION_HISTORY_OK:True`、`SCOPE_BREADCRUMB_NAVIGATION_OK:True` 和 `FOCUS_RESTORE_OK:True`
