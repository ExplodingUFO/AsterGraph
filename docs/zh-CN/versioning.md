...
仓库现在已经开始发布和包版本一致的公开 prerelease tag，例如 `v0.10.0-beta`。

仓库里也仍保留着像 `v1.9` 这样的历史 tag。

这些 `v1.x` tag 来自公开前的 milestone 式仓库检查点，适合维护者回看历史，但**不是**外部用户在 NuGet 上安装的包版本。

后续对外版本约定是：

- 公开发布包使用与包 SemVer 对齐的 tag，例如 `v0.10.0-beta`
...
| 对外概念 | 当前值 | 应该怎么理解 |
| --- | --- | --- |
| 可安装包版本 | `0.10.0-beta` | 外部用户从 nuget.org 安装的版本 |
| 当前对外 prerelease tag | `v0.10.0-beta` | GitHub 上与可安装包版本严格对齐的公开 tag |
