...
Phase 529 记录 whiteboard/lasso/eraser feasibility audit，对应 GitHub #181 / `avalonia-node-map-jrm`。Phase 530 通过 GitHub #183 / `avalonia-node-map-8um` 新增 backend/editor lasso selection query。Phase 531 通过 GitHub #185 / `avalonia-node-map-084` 新增 internal Avalonia lasso selection bridge。Phase 532 通过 GitHub #187 / `avalonia-node-map-mka` 新增 internal lasso gesture capture。React Flow 在 https://reactflow.dev/learn/advanced-use/whiteboard 把 whiteboard features 作为单独 advanced track 说明，包括 Freehand draw、Lasso selection、Eraser 和 Rectangle draw。AsterGraph 当前 evidence 更窄：rectangular marquee selection、backend lasso selection query、internal lasso bridge、internal lasso gesture capture 和 selection command projection 有 guard，但它不等同于 full React Flow whiteboard parity，也不承诺本 slice 之外的 public UI/renderer 行为。
