...
| Marquee/box selection and multi-select | Present | `GetSelectionRectangleSnapshot`、`GetSelectionLassoSnapshot`、overlay coordinator marquee selection、internal lasso bridge selection、`SelectAll`、`SelectNone`、`InvertSelection` 有 tests 和 Cookbook routes。Rectangle marquee 已有 UI-backed route；lasso/freehand 有 backend/editor query 与 internal Avalonia bridge，但还不是 public pointer-mode UI。 | Keep guarded。 |
| Whiteboard / lasso / eraser / rectangle draw | Gap retained / internal lasso bridge started | React Flow whiteboard docs 把 Freehand draw、Lasso selection、Eraser 和 Rectangle draw 列为 distinct advanced examples。AsterGraph 现在有 rectangle marquee selection、command projection evidence、受防守的 backend `GetSelectionLassoSnapshot(...)` query 和 internal Avalonia lasso selection bridge，但没有 public UI pointer mode、whiteboard primitives 或 drawing tools。 | Phase 531 只桥接 lasso selection finalize，不实现 full whiteboard UI behavior。 |
...
Phase 529 记录 whiteboard/lasso/eraser feasibility audit，对应 GitHub #181 / `avalonia-node-map-jrm`。Phase 530 通过 GitHub #183 / `avalonia-node-map-8um` 新增 backend/editor lasso selection query。Phase 531 通过 GitHub #185 / `avalonia-node-map-084` 新增 internal Avalonia lasso selection bridge。React Flow 在 https://reactflow.dev/learn/advanced-use/whiteboard 把 whiteboard features 作为单独 advanced track 说明，包括 Freehand draw、Lasso selection、Eraser 和 Rectangle draw。AsterGraph 当前 evidence 更窄：rectangular marquee selection、backend lasso selection query、internal lasso bridge 和 selection command projection 有 guard，但它不等同于 full React Flow whiteboard parity，也不承诺本 slice 之外的 public UI/renderer 行为。
