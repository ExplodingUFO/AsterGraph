...
- `tests/AsterGraph.Demo.Tests/CookbookShellVisualGateStates.json` 当前定义 `shell-cookbook-default-open`、`shell-cookbook-default-open-zh-cn`、`shell-cookbook-default-closed` 和 `shell-runtime-diagnostics-open`。
...
| Demo/Cookbook | `src/AsterGraph.Demo` | 25 个 recipes、route clarity docs、built-in/interaction/lifecycle batches、scene PNG screenshot gate metadata，以及四条 manifest-driven full-window shell visual states。 | Cookbook breadth、scene coverage、English default open/closed shell captures、English runtime drawer capture 和 Chinese Cookbook drawer capture 已覆盖。Pixel-baseline comparison 以及更广的 flyout/popup/context-menu/theme/language shell-state coverage 仍作为后续有界工作。 |
...
| Screenshot-driven UI quality | Partial / guarded | Scene PNG gate 已覆盖规范 graph scenes。Full-window shell gate 现在会捕获 English default Cookbook drawer、Chinese default Cookbook drawer 和 English runtime diagnostics drawer states，包含 host menu、drawer、graph host、named shell parts、选中的 language/theme metadata 和 artifact metadata。Pixel-baseline comparison 与更广的 flyout/popup/context-menu/theme/language coverage 尚未覆盖。 | Phase 494 保持在 manifest-driven shell states 范围内；只有 visual drift evidence 需要时再追加更广的 baseline follow-up。 |
...
Phase 495 在 GitHub 与 Beads 都回到零 open issues 后刷新 active queue。Phase 496 已关闭 Cookbook example architecture contract。Phase 497 现在通过 GitHub #117 / `avalonia-node-map-5nl` 承接 closed-drawer shell visual gate breadth slice；它是 current/owned work，不是 broad shell certification。

| GitHub | Bead | 标题 | 优先级 | 可能 write set | 并行边界 |
| --- | --- | --- | --- | --- | --- |
| #117 | `avalonia-node-map-5nl` | Phase 497: expand shell visual gate with closed drawer state | P2 | `tests/AsterGraph.Demo.Tests/CookbookShellVisualGateStates.json`、`tests/AsterGraph.Demo.Tests/DemoCookbookScreenshotGateTests.cs`、双语 screenshot docs | Current owned slice。只为 default route 新增 `expectedPaneOpen: false` 的 `shell-cookbook-default-closed`；仍不做 strict pixel baselines、flyout capture、popup capture、context-menu capture 或 broad language/theme certification。 |
...
| TBD | TBD | Phase 500: choose next shell visual breadth state | P3 | 仅 shell manifest/docs/tests | Phase 497 关闭后再选择一个有界 state，例如 selected non-Cookbook host group 或 search/filter state；flyouts、popups、context menus、broad language/theme certification 和 strict pixel baselines 继续不纳入范围。 |
...
- `docs/phase-497-shell-closed-drawer-gate`：负责 #117 / `avalonia-node-map-5nl`；写集限制在 shell manifest、screenshot tests 和 screenshot docs。
...
当前覆盖包含 scene-level route captures 和四条 manifest-driven full-window shell captures。`DemoCookbookScreenshotGateTests`、`CookbookScreenshotGateRoutes.json` 和 `CookbookShellVisualGateStates.json` 能证明规范 graph scenes、route metadata、English default Cookbook open artifact、English default Cookbook closed-drawer artifact、Chinese default Cookbook shell artifact 和 English runtime diagnostics shell artifact；它们仍不提供严格 pixel-baseline comparisons 或广泛 flyout/popup/context-menu/theme/language coverage。
...
- Phase 478、Phase 484、Phase 490、Phase 491、Phase 492、Phase 493、Phase 494、Phase 495 和 Phase 497 都不修改产品代码；除非 focused test 证明存在具体 missing contract。
