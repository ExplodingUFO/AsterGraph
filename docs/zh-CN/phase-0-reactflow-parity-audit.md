...
| Marquee/box selection and multi-select | Present | `GetSelectionRectangleSnapshot`、`GetSelectionLassoSnapshot`、overlay coordinator marquee selection、`SelectAll`、`SelectNone`、`InvertSelection` 有 tests 和 Cookbook routes。Rectangle marquee 已有 UI-backed route；lasso/freehand 目前仅是 backend/editor query。 | Keep guarded。 |
| Whiteboard / lasso / eraser / rectangle draw | Gap retained / backend lasso query started | React Flow whiteboard docs 把 Freehand draw、Lasso selection、Eraser 和 Rectangle draw 列为 distinct advanced examples。AsterGraph 现在有 rectangle marquee selection、command projection evidence 和受防守的 backend `GetSelectionLassoSnapshot(...)` query，但没有 UI pointer mode、whiteboard primitives 或 drawing tools。 | Phase 530 只启动 lasso query contract，不实现 whiteboard UI behavior。 |
...
Phase 529 记录 whiteboard/lasso/eraser feasibility audit，对应 GitHub #181 / `avalonia-node-map-jrm`。Phase 530 通过 GitHub #183 / `avalonia-node-map-8um` 新增 backend/editor lasso selection query。React Flow 在 https://reactflow.dev/learn/advanced-use/whiteboard 把 whiteboard features 作为单独 advanced track 说明，包括 Freehand draw、Lasso selection、Eraser 和 Rectangle draw。AsterGraph 当前 evidence 更窄：rectangular marquee selection、backend lasso selection query 和 selection command projection 有 guard，但它不等同于 full React Flow whiteboard parity，也不承诺本 slice 之外的 runtime/UI/renderer 行为。
...
| Lasso/freehand selection | Backend query supported / UI gap retained | `GetSelectionLassoSnapshot`、`GraphEditorSelectionLassoSnapshot`、`Queries_GetSelectionLassoSnapshot_ReturnsCenterContainedNodesAndConnections`、`Queries_GetSelectionLassoSnapshot_UsesNodeCenterInsteadOfBoundsIntersection` 和 `Queries_GetSelectionLassoSnapshot_WithOpenOrDegeneratePath_ReturnsDeterministicResults` 证明 active-scope backend query。该 query 把 freeform points 当作 closed polygon，用 center-point containment 选择 node，并在两个 endpoint node 都选中时选择 connection。 | 尚未实现 Avalonia pointer-mode state machine、gesture capture、lasso screenshot route、eraser behavior、drawing primitives、persistence 或 renderer layer。Phase 530 总体边界仍是 not full React Flow whiteboard parity。 |
