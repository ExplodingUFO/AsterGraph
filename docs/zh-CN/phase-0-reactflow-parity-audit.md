...
| TBD | TBD | Phase 547: whiteboard primitive model skeleton | P2 | Core/Editor primitive model tests、internal model types 和 renderer-neutral contract docs | Phase 542-546 gate stack lands 之后的 candidate。不得触碰 renderer adapters、persistence/schema、pointer coordinators、screenshot manifests、toolbar UX 或 eraser behavior。 |
...
- `feature/phase-547-whiteboard-primitive-model-skeleton`：future candidate，用于 gate stack lands 后的第一条 whiteboard primitive model skeleton。
