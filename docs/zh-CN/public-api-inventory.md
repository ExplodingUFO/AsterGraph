...
| `AsterGraph.Avalonia` | canonical editor/session route 上的 adapter 投影，以及复用同一 runtime owner 的 hosted factory。 | `AsterGraphAvaloniaViewFactory.Create(...)`、`AsterGraphCanvasViewFactory`、`AsterGraphInspectorViewFactory`、`AsterGraphMiniMapViewFactory`、`NodeToolbar`、`EdgeToolbar`、`NodeResizer`、`AsterGraphHostBuilder`、`AsterGraphWorkbenchOptions`、`AsterGraphBuiltInComponentCatalog`、hosted workbench layout / panel-state options，以及 hosted performance policy budget markers。 | 仍使用 retained editor facade 的宿主可嵌入 `GraphEditorView`。 | 仅限把现有宿主桥接到 canonical route 的 adapter-specific compatibility glue。 | Control internals、templates、interaction session internals、visual-only implementation details。 |
...
| Built-in component catalog | Supported hosted helper | `AsterGraphBuiltInComponentCatalog.Components`、`AsterGraphBuiltInComponentCatalog.TryGet(...)`、`AsterGraphBuiltInComponentDescriptor`、`AsterGraphBuiltInComponentStatus`、`NodeToolbar`、`EdgeToolbar`、`NodeResizer` | Phase 3 的内置 component track discovery map。Public 条目指向现有 canvas、minimap、background-grid、inspector、controls-panel、command-tool-projection、`node-toolbar`、`edge-toolbar` 和 `node-resizer` surface。`NodeToolbar` 与 `EdgeToolbar` 是基于 `AsterGraphAuthoringToolActionFactory.CreateNodeActions(...)` 和 `CreateConnectionActions(...)` 的薄 Avalonia 控件；`NodeResizer` 是基于 `IGraphEditorSession.Commands.TrySetNodeSize(...)` 的薄 Avalonia 控件。 |
