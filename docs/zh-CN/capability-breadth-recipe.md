...
把它和 `src/AsterGraph.Demo` 配在一起，因为这就是当前 beta evaluator ladder 里受防守的 hosted proof route。
...
- 第 1 步：通过 `src/AsterGraph.Demo`、`IGraphEditorSession.Queries.GetNodeTemplateSnapshots()` 和 stock hosted library chrome 投影 searchable grouped stencil。
...
- 第 3 步：通过 `AsterGraphAuthoringToolActionFactory.CreateNodeActions(...)`、`AsterGraphAuthoringToolActionFactory.CreateConnectionActions(...)`、`AsterGraphAuthoringToolActionFactory.CreateCommandSurfaceActions(...)`、`IGraphEditorSession.Queries.GetToolDescriptors(...)` 和 `src/AsterGraph.Demo -- --proof` 投影共享 node/edge quick tools。
