...
Use `AsterGraph.Starter.Avalonia` for the first hosted entry. Use `AsterGraph.HelloWorld` for the runtime-only first run. Use `AsterGraph.HostSample` when you need the proof harness for canonical route validation. Use [Authoring Inspector Recipe](../../docs/en/authoring-inspector-recipe.md) when you want to reproduce the same definition-driven inspector pattern in your own host.
